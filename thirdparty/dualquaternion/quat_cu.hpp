...
class Quat_cu{
    ...
    inline __host__ __device__ Quat_cu()
    {
        coeff[0] = 1.f;
    ...
    /// Copy constructor
    inline __host__ __device__ Quat_cu(const Quat_cu& q){
        coeff[0] = q.w();
        coeff[1] = q.i(); coeff[2] = q.j(); coeff[3] = q.k();
    }

    /// directly fill the quaternion
    inline __host__ __device__ Quat_cu(float w, float i, float j, float k){
        coeff[0] = w;
        coeff[1] = i; coeff[2] = j; coeff[3] = k;
    }

    /// directly fill the quaternion vector and scalar part
    inline __host__ __device__ Quat_cu(float w, const Vec3& v){
        coeff[0] = w;
    ...
    inline __host__ __device__ Quat_cu(const Transfo& t)
    {
    ...
    /// 'angle' in radians
    inline __host__ __device__ Quat_cu(const Vec3& axis, float angle)
    {
    ...
    inline __host__ __device__ Quat_cu conjugate() const
    {
    ...
    inline __host__ __device__ Point3 rotate(const Point3& p) const
    {
    ...
    inline __host__ __device__ Mat3 to_matrix3()
    {
    ...
    inline __host__ __device__ Vec3 get_vec_part() const
    {
    ...
    inline __host__ __device__ float dot(const Quat_cu& q){
        return w() * q.w() + i() * q.i() + j() * q.j() + k() * q.k();
    }

    inline __host__ __device__ float w() const { return coeff[0]; }
    inline __host__ __device__ float i() const { return coeff[1]; }
    inline __host__ __device__ float j() const { return coeff[2]; }
    inline __host__ __device__ float k() const { return coeff[3]; }

    // -------------------------------------------------------------------------
    /// @name Operators
    // -------------------------------------------------------------------------

    inline __host__ __device__ Quat_cu operator/ (float scalar) const
    {
        Quat_cu q = *this;
        q.coeff[0] /= scalar;
        q.coeff[1] /= scalar;
    ...
    inline __host__ __device__ Quat_cu operator/= (float scalar){
        coeff[0] /= scalar;
        coeff[1] /= scalar;
        coeff[2] /= scalar;
        coeff[3] /= scalar;
    ...
    inline __host__ __device__ Quat_cu operator* (const Quat_cu& q) const
    {
         return Quat_cu(
