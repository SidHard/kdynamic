...
struct Vec3 {
    ...
    inline __host__ __device__ Vec3() { x = 0.f; y = 0.f; z = 0.f; }

    inline __host__ __device__ Vec3(float x_, float y_, float z_) { x = x_; y = y_; z = z_; }

    inline __host__ __device__ Vec3(float v) { x = v; y = v; z = v; }
    ...
    inline __host__ __device__ explicit Vec3(const Point3& p3);

    static inline __host__ __device__ Vec3 unit_x()    { return Vec3(1.f, 0.f, 0.f); }
    static inline __host__ __device__ Vec3 unit_y()    { return Vec3(0.f, 1.f, 0.f); }
    static inline __host__ __device__ Vec3 unit_z()    { return Vec3(0.f, 0.f, 1.f); }
    static inline __host__ __device__ Vec3 zero()      { return Vec3(0.f, 0.f, 0.f); }
    static inline __host__ __device__ Vec3 unit_scale(){ return Vec3(1.f, 1.f, 1.f); }
    ...
    inline __host__ __device__ Vec3 operator+(const Vec3 &v_) const { return Vec3(x+v_.x, y+v_.y, z+v_.z); }

    inline __host__ __device__ Vec3& operator+= (const Vec3 &v_) {
        x += v_.x;
        y += v_.y;
        z += v_.z;
        return *this;
    ...
    inline __host__ __device__ Vec3 operator+(float f_) const { return Vec3(x+f_, y+f_, z+f_); }
    ...
    inline __host__ __device__ Vec3& operator+= (float f_) {
        x += f_;
        y += f_;
        z += f_;
        return *this;
    ...
    inline __host__ __device__ Vec3 operator-(const Vec3 &v_) const {
    ...
    inline __host__ __device__ bool operator!= (const Vec3 &v_) const {
        return (x != v_.x) |  (y != v_.y) | (z != v_.z);
    }

    inline __host__ __device__ bool operator==(const Vec3& d_) const {
    ...
    inline __host__ __device__ Vec3 operator/(const float d_) const {
        return Vec3(x/d_, y/d_, z/d_);
    }

    inline __host__ __device__ Vec3& operator/=(const float d_) {
        x /= d_;
        y /= d_;
        z /= d_;
        return *this;
    ...
    inline __host__ __device__ Vec3& operator/=(const Vec3& d_) {
        x /= d_.x;
        y /= d_.y;
        z /= d_.z;
        return *this;
    ...
    inline __host__ __device__ Vec3 operator*(const float d_) const { return Vec3(x*d_, y*d_, z*d_); }
    ...
    inline __host__ __device__ float product() const { return x*y*z; }

    /// sum of all components
    inline __host__ __device__ float sum() const { return x+y+z; }

    /// Average all components
    inline __host__ __device__ float average() const { return (x+y+z)/3.f; }

    /// semi dot product (component wise multiplication)
    inline __host__ __device__ Vec3 mult(const Vec3& v) const {
        return Vec3(x*v.x, y*v.y, z*v.z);
    }

    /// component wise division
    inline __host__ __device__ Vec3 div(const Vec3& v) const {
        return Vec3(x/v.x, y/v.y, z/v.z);
    }

    /// cross product
    inline __host__ __device__ Vec3 cross(const Vec3& v_) const {
        return Vec3(y*v_.z-z*v_.y, z*v_.x-x*v_.z, x*v_.y-y*v_.x);
    }

    /// dot product
    inline __host__ __device__ float dot(const Vec3& v_) const {
        return x*v_.x+y*v_.y+z*v_.z;
    }

    /// Compute the cotangente (i.e. 1./tan) between 'this' and v_
    inline __host__ __device__ float cotan(const Vec3& v_) const {
    ...
    inline __host__ __device__ float signed_angle(const Vec3& v1, const Vec3& v2) const {
        return atan2(  this->dot( v1.cross(v2) ), v1.dot(v2) );
    }

    /// absolute value of the dot product
    inline __host__ __device__ float abs_dot(const Vec3& v_) const {
        return fabsf(x * v_.x + y * v_.y + z * v_.z);
    }

    /// norm squared
    inline __host__ __device__ float norm_squared() const {
        return dot(*this);
    }
    ...
    inline __host__ __device__ Vec3 get_ortho() const
    {
    ...
    /// proj_on_plane() !
    inline __host__ __device__ Vec3 proj_on_plane(const Vec3& to_project) const
    {
    ...
    inline __host__ __device__ Point3 proj_on_plane(const Point3& pos_plane,
    ...
    /// @note implemented in Point3.hpp because of cross definitions
    inline __host__ __device__ Point3 to_point3() const;
    ...
    /// glVertex3fv(pos);
    /// @endcode
    inline __host__ __device__ operator const float*() const { return &x; }

    /// Conversion returns the memory address of the vector. (Non const version)
    inline __host__ __device__ operator float*() { return &x; }
