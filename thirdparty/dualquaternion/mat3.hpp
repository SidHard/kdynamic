...
struct Mat3 {
    ...
    inline __host__ __device__ Mat3() {   }
    ...
    inline __host__ __device__ Mat3 operator*(const Mat3& m) const
    {
    ...
    inline __host__ __device__ Mat3 operator*(float x) const
    {
    ...
    inline __host__ __device__ Mat3& operator*=(float x)
    {
        a *= x; b *= x; c *= x;
        d *= x; e *= x; f *= x;
        g *= x; h *= x; i *= x;
    ...
    inline friend __host__ __device__  Mat3 operator*(const float x_, const Mat3& mat)
    {
    ...
    */
    ...
    inline __host__ __device__ const float& operator()(int row, int column) const
    {
        assert(row >= 0 && row < 3);
    ...
    inline __host__ __device__ float& operator()(int row, int column)
    {
        assert(row >= 0 && row < 3);
        assert(column >= 0 && column < 3);
        return ((float*)(this))[column + row*3];
    ...
    inline __host__ __device__ float& operator[](int idx) {
        assert(idx >= 0 && idx < 9);
        return ((float*)(this))[idx];
    }

    inline __host__ __device__ const float& operator[](int idx) const {
        assert(idx >= 0 && idx < 9);
        return ((float*)(this))[idx];
    }
    ...
    inline __host__ __device__ Mat3 normalized() const {
        return Mat3(x().normalized(), y().normalized(), z().normalized());
    }

    inline __host__ __device__ Mat3 inverse() const
    {
        float c0 = e * i - f * h;
        float c1 = f * g - d * i;
        float c2 = d * h - e * g;
    ...
    inline __host__ __device__ Mat3 transpose() const
    {
        return Mat3(a, d, g, b, e, h, c, f, i);
    }

    inline __host__ __device__ void set_abs()
    {
    ...
    inline __host__ __device__ float get_rotation_axis_angle(Vec3& axis) const
    {
        axis.x = h - f + 1e-5f;
        axis.y = c - g;
        axis.z = d - b;
    ...
    inline __host__ __device__ Vec3 x() const { return Vec3(a, d, g); }

    inline __host__ __device__ Vec3 y() const { return Vec3(b, e, h); }

    inline __host__ __device__ Vec3 z() const { return Vec3(c, f, i); }

    //--------------------------------------------------------------------------
    /// @name Static constructors
    //--------------------------------------------------------------------------

    static inline __host__ __device__ Mat3 identity()
    {
    ...
    static inline __host__ __device__ Mat3 rotate(const Vec3& axis, float angle)
    {
        Vec3 n = axis;
        n.normalize();
        float cp = cosf(angle);
    ...
    static inline __host__ __device__ Mat3 coordinate_system(const Vec3& x_axis)
    {
        Vec3 fx, fy, fz;
