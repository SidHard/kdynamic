...
struct Transfo {
    ...
    inline __host__ __device__ Transfo() {}


    inline __host__ __device__ Transfo(float a00, float a01, float a02, float a03,
    ...
    inline __host__ __device__ Transfo(const Mat3& x){
    ...
    inline __host__ __device__ Transfo(const Mat3& x, const Vec3& v){
    ...
    inline __host__ __device__ Transfo(const Vec3& v){
    ...
    inline __host__ __device__ Vec3 x() const{ return Vec3( m[0], m[4], m[ 8] ); }
    inline __host__ __device__ Vec3 y() const{ return Vec3( m[1], m[5], m[ 9] ); }
    inline __host__ __device__ Vec3 z() const{ return Vec3( m[2], m[6], m[10] ); }


    inline __host__ __device__ Mat3 get_mat3() const {
    ...
    inline __host__ __device__ Vec3 get_translation() const { return Vec3(m[3], m[7], m[11]); }

    /// get origine of the frame represented by the matrix
    /// @note same as get_translation()
    inline __host__ __device__ Vec3 get_org() const { return Vec3(m[3], m[7], m[11]); }

    inline __host__ __device__ void set_x(const Vec3& x){ m[0] = x.x; m[4] = x.y; m[ 8] = x.z; }
    inline __host__ __device__ void set_y(const Vec3& y){ m[1] = y.x; m[5] = y.y; m[ 9] = y.z; }
    inline __host__ __device__ void set_z(const Vec3& z){ m[2] = z.x; m[6] = z.y; m[10] = z.z; }

    inline __host__ __device__ void set_translation(const Vec3& tr){ m[3] = tr.x; m[7] = tr.y; m[11] = tr.z; }
    inline __host__ __device__ void set_org        (const Vec3& tr){ m[3] = tr.x; m[7] = tr.y; m[11] = tr.z; }


    inline __host__ __device__ void set_translation(const Transfo& tr){
        const Vec3 trans = tr.get_translation();
        m[3] = trans.x; m[7] = trans.y; m[11] = trans.z;
    }


    inline __host__ __device__ void set_mat3(const Mat3& x){
    ...
    /// @warning undefined behavior if 'this' is not an affine transformation
    inline __host__ __device__ Point3 operator*(const Point3& v) const {
        return Point3(
    ...
    inline __host__ __device__ Point3 project(const Point3& v) const {
        Point3 tmp =  Point3(
                    m[0] * v.x + m[1] * v.y + m[ 2] * v.z + m[ 3],
                    m[4] * v.x + m[5] * v.y + m[ 6] * v.z + m[ 7],
                    m[8] * v.x + m[9] * v.y + m[10] * v.z + m[11]);
    ...
    inline __host__ __device__ Transfo operator*(const Transfo& t) const {
        Transfo res;
        for(int i = 0; i < 4; i++){
            int j = i*4;
    ...
    inline __host__ __device__ Transfo& operator*=(const Transfo& t){
        (*this) = (*this) * t;
        return (*this);
    }
    ...
    /// for a vector) you may use #Vec4 for a full multiplication.
    /// @warning undefined behavior if 'this' is not an affine transformation
    inline __host__ __device__ Vec3 vec_prod(const Vec3& v) const {
        return Vec3(
                    m[0] * v.x + m[1] * v.y + m[ 2] * v.z,
                    m[4] * v.x + m[5] * v.y + m[ 6] * v.z,
                    m[8] * v.x + m[9] * v.y + m[10] * v.z);
    ...
    inline __host__ __device__ Transfo operator*(float x) const {
        Transfo res;
        for(int i = 0; i < 16; i++){
            res[i] = m[i] * x;
        }
    ...
    inline __host__ __device__ Transfo transpose() const {
    ...
    /// @see full_invert()
    inline __host__ __device__ Transfo fast_invert() const {
        Mat3 a(m[0], m[1], m[ 2],
               m[4], m[5], m[ 6],
               m[8], m[9], m[10]);
    ...
    /// @see fast_invert()
    inline __host__ __device__ Transfo full_invert() const {
        return adjoint() * (1.0f / det());
    }

    /// @return the Transformation with normalized x, y, z column vectors
    inline __host__ __device__ Transfo normalized() const {
    ...
    inline __host__ __device__ bool is_frame_ortho(float eps = 0.0001f) const {
...
//    inline __host__ __device__ friend
...
//    }
    ...
    static inline __host__ __device__ Transfo translate(float dx, float dy, float dz){
    ...
    static inline __host__ __device__ Transfo translate(const Vec3& v){
        return Transfo::translate(v.x, v.y, v.z);
    }


    static inline __host__ __device__ Transfo scale(float sx, float sy, float sz){
    ...
    static inline __host__ __device__ Transfo scale(const Vec3& v){
        return Transfo::scale(v.x, v.y, v.z);
    }

    /// Build a uniform scaling matrix on x,y,z.
    static inline __host__ __device__ Transfo scale(float s){
        return Transfo::scale(s, s, s);
    }

    /// @return the scale matrix given scale factors in 'v' and
    /// scale origin 'center'
    static inline __host__ __device__ Transfo scale(const Vec3& center, const Vec3& v)
    {
    ...
    /// scale origin 'center'
    static inline __host__ __device__ Transfo scale(const Vec3& center, float s)
    {
    ...
    static inline __host__ __device__ Transfo rotate(const Vec3& center,
                                 const Vec3& axis,
                                 float angle,
                                 const Mat3& frame)
    {
    ...
    static inline __host__ __device__ Transfo rotate(const Vec3& axis, float angle)
    {
        return Transfo(Mat3::rotate(axis, angle));
    }


    static inline __host__ __device__ Transfo identity(){
    ...
    static inline __host__ __device__ Transfo empty(){
    ...
    static inline __host__ __device__ Transfo coordinate_system(const Vec3& org, const Vec3& x_axis){
